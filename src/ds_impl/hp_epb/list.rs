...
use hp_epb::{decompose_ptr, domain, tag, untagged, HazardPointer, Thread};
...
impl Default for Handle<'static> {
    fn default() -> Self {
        let mut thread = Box::new(Thread::new(domain()));
        Self {
    ...
    K: Ord,
{
    ...
    fn find_harris_michael(&mut self, key: &K) -> Result<bool, ()> {
        loop {
            ...
            self.handle.thread.light_fence();
...
mod tests {
    use super::HMList;
    use crate::ds_impl::hp_epb::concurrent_map;
