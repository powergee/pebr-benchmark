use hp_epb::{domain, set_counts_between_flush};
...
use smr_benchmark::ds_impl::hp_epb::{ConcurrentMap, HMList, HashMap};
...
fn bench_map<M: ConcurrentMap<usize, usize> + Send + Sync>(
    ...
    match config.bag_size {
        BagSize::Small => set_counts_between_flush(128),
            ...
            s.spawn(move |_| {
                ...
                while start.elapsed() < config.duration {
                    ...
                    if now > next_sampling {
                        ...
                        let garbages = domain().num_garbages();
                        garb_acc += garbages;
